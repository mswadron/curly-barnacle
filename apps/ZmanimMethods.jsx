  ...
  const useMyLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        const near = best ? (bestD < 40 ? " · near " + best.name : " · nearest " + best.name + " (" + Math.round(bestD) + " km)") : "";
        setGeo({ ok: true, text: "Located: " + Math.abs(lat).toFixed(4) + "\u00B0" + (lat >= 0 ? "N" : "S") + ", " + Math.abs(lon).toFixed(4) + "\u00B0" + (lon >= 0 ? "E" : "W") + " \u00B7 " + tz + (acc ? " \u00B7 \u00B1" + acc + " m" : "") + near });
      },
      (err) => {
