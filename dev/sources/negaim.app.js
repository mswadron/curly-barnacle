...
{(type==="mishnah"||type==="rambam")&&<div style={{fontSize:11,color:"var(--t3)",direction:"rtl",textAlign:"right",margin:"-6px 0 10px"}}>{tx(lang,'הנוסח כאן ע"פ המקור — הלשון המלא בקישור',"Text follows the source — full wording at the link")}</div>}
{su&&<a href={su} target="_blank" rel="noopener noreferrer" style={{display:"inline-block",fontFamily:"var(--fm)",fontSize:11,color:"var(--ac)",textDecoration:"none",borderBottom:"1px dotted var(--ac)",marginBottom:8}}>{tx(lang,"לצפייה במקור ↗","View source ↗")}</a>}
